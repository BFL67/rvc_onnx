...
def temperature_sigmoid(x, x0, temp_coeff):
    """Applies a temperature-controlled sigmoid function."""
...
class SpectralGate:
    """Applies a spectral gate to audio."""
    def __init__(self, y, sr, prop_decrease, chunk_size, padding, n_fft, win_length, hop_length, time_constant_s, freq_mask_smooth_hz, time_mask_smooth_ms, tmp_folder, use_tqdm, n_jobs):
        ...
        if len(y.shape) == 1:
            ...
            self.flat = True
        elif len(y.shape) > 2: raise ValueError("Waveform must have shape (# frames, # channels)")
        else: self.y = y
    ...
    def _generate_mask_smoothing_filter(self, freq_mask_smooth_hz, time_mask_smooth_ms):
        if freq_mask_smooth_hz is None: n_grad_freq = 1
            ...
            if n_grad_freq < 1: raise ValueError(f"freq_mask_smooth_hz must be at least {int((self.sr / (self._n_fft / 2)))}Hz")

        if time_mask_smooth_ms is None: n_grad_time = 1
        else:
            n_grad_time = int(time_mask_smooth_ms / ((self._hop_length / self.sr) * 1000))
            if n_grad_time < 1: raise ValueError(f"time_mask_smooth_ms must be at least {int((self._hop_length / self.sr) * 1000)}ms")
...
class TG(torch.nn.Module):
    ...
    def forward(self, x, xn = None):
        assert x.ndim == 2
        if x.shape[-1] < self.win_length * 2: raise Exception(f"xn must be greater than {self.win_length * 2}")
        assert xn is None or xn.ndim == 1 or xn.ndim == 2
        if xn is not None and xn.shape[-1] < self.win_length * 2: raise Exception(f"xn must be greater than {self.win_length * 2}")
...
class StreamedTorchGate(SpectralGate):
    """Streamed spectral gate using PyTorch."""
