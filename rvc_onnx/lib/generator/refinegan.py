...
from lib.generator.commons import get_padding

class ResBlock(torch.nn.Module):
    """Residual Block for RefineGAN."""
...
class ParallelResBlock(torch.nn.Module):
    """Parallel Residual Block for RefineGAN."""
...
class SineGenerator(torch.nn.Module):
    """Generates sine waves based on F0 values for RefineGAN."""
...
class RefineGANGenerator(torch.nn.Module):
    ...
    def forward(self, mel, f0, g = None):
        ...
        for ups, res, down, flt in zip(self.upsample_blocks, self.upsample_conv_blocks, self.downsample_blocks, self.filters):
            x = checkpoint.checkpoint(res, torch.cat([checkpoint(flt, checkpoint(ups, x, use_reentrant=False), use_reentrant=False), down(har_source)], dim=1), use_reentrant=False) if self.training and self.checkpointing else res(torch.cat([flt(ups(x)), down(har_source)], dim=1))
