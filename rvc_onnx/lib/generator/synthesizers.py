...
from lib.generator.modules import WaveNet
from lib.generator.refinegan import RefineGANGenerator
from lib.generator.mrf_hifigan import HiFiGANMRFGenerator
from lib.generator.residuals import ResidualCouplingBlock, ResBlock, LRELU_SLOPE
from lib.generator.commons import init_weights, sequence_mask
...
class SourceModuleHnNSF(torch.nn.Module):
    """Source module for Harmonic-plus-Noise Sine-based F0 (HnNSF)."""
...
class GeneratorNSF(torch.nn.Module):
    """Generator with Neural Source Filter (NSF)."""
...
class TextEncoder(torch.nn.Module):
    """Text Encoder module."""
    def __init__(self, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout=0.0, gin_channels=0):
        super().__init__()
        self.out_channels = out_channels
        self.hidden_channels = hidden_channels
        self.filter_channels = filter_channels
        ...
        self.emb_g = torch.nn.Embedding(gin_channels, hidden_channels) if gin_channels != 0 else None
    ...
    def forward(self, x, x_lengths, g=None):
        ...
        if g is not None: g = self.emb_g(g).unsqueeze(-1)

        m, logs = torch.split(self.proj(x), self.out_channels, dim=1)
        return m, logs, x_mask, x, g


class SynthesizerTrn(torch.nn.Module):
    """Synthesizer training module."""
    def __init__(self, n_vocab, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, n_speakers, gin_channels, use_sdp=True, **kwargs):
        super().__init__()
        ...
        self.enc_p = TextEncoder(inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, gin_channels=gin_channels)
        ...
        self.flow = ResidualCouplingBlock(inter_channels, hidden_channels, kernel_size, p_dropout, n_layers, n_flows=4, gin_channels=gin_channels)

    def forward(self, x, x_lengths, sid=None):
        m_p, logs_p, x_mask, x, g = self.enc_p(x, x_lengths, sid)
        z, logs_q = self.flow(m_p, x_mask, g=g)
        o = self.dec(z, g=g)
        return o, logs_q, x_mask, (m_p, logs_p, x)

    def infer(self, x, x_lengths, sid=None, noise_scale=0.35, length_scale=1, noise_scale_w=0.35, max_len=None):
        m_p, logs_p, x_mask, x, g = self.enc_p(x, x_lengths, sid)
        z = self.flow(m_p, x_mask, g=g, reverse=True)
        o = self.dec(z * noise_scale, g=g)
        return o


class SynthesizerONNX(torch.nn.Module):
    """ONNX-compatible Synthesizer module."""
    def __init__(self, n_vocab, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, n_speakers, gin_channels, use_f0=1, vocoder="HifiGAN", checkpointing=False):
        super().__init__()
        self.n_vocab = n_vocab
        self.spec_channels = spec_channels
        self.segment_size = segment_size
        ...
        if vocoder == "HifiGAN":
            self.dec = HiFiGANMRFGenerator(inter_channels, upsample_initial_channel, upsample_rates, upsample_kernel_sizes, resblock_kernel_sizes, resblock_dilation_sizes, gin_channels, 44100, 0, checkpointing=checkpointing)
        elif vocoder == "RefineGAN":
            self.dec = RefineGANGenerator(44100, upsample_rates, LRELU_SLOPE, inter_channels, gin_channels, checkpointing=checkpointing)
        elif vocoder == "NSF":
            self.dec = GeneratorNSF(inter_channels, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels, 44100, checkpointing=checkpointing)
        else:
            self.dec = Generator(inter_channels, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=gin_channels)

    def forward(self, phone, phone_lengths, ds, rnd, pitch=None, pitchf=None):
        g = self.enc_p.emb_g(ds).unsqueeze(-1) if self.gin_channels != 0 else None
        m_p, logs_p, x_mask, x, g = self.enc_p(phone, phone_lengths, ds)
        z = self.flow(m_p, x_mask, g=g, reverse=True)
    ...
    def __prepare_scriptable__(self):
        self.dec.__prepare_scriptable__()
        self.flow.__prepare_scriptable__()
        return self
